...
export default function EditShowTiming() {
    ...
    const { data: showTimingDetail } = showTimingsApi.useDetail(showTimingId);

    const { data: moviesList } = moviesApi.useList();

    const { data: screensList } = screensApi.useList();

    const { mutateAsync: updateShowTime } = showTimingsApi.useUpdate(showTimingId);
    ...
    useEffect(() => {
        if (showTimingDetail) {
            reset({
                movie_id: showTimingDetail.movie.id,
                screen_id: showTimingDetail.screen.id,
                date: showTimingDetail.date,
                time: showTimingDetail.time
            });
        }
    }, [reset, showTimingDetail]);

    const onSubmit = async data => {
        const response = await updateShowTime(data);
        if (response.data) {
            reset();
    ...
    return (
        <>
            <CinewaveNavbar title="Edit Show Timing" hideSearch />
            <br />
            ...
            <Container>
                <Breadcrumb>
                    ...
                    <Breadcrumb.Item active>Edit Show Time</Breadcrumb.Item>
                </Breadcrumb>
                ...
                <Row>
                    <Col>
                        <Card>
                            <Card.Body>
                                <Card.Title>Edit Show Timing</Card.Title>
                            </Card.Body>
